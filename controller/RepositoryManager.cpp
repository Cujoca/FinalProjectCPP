...
bool RepositoryManager::createAndTrackFile(const string filePath, const string content) {
...
bool RepositoryManager::commitChanges(const string message, const string author) {
    ...
    return succeed("Created commit " + created->getCommitID() + " — " + created->getSummary() + ".");
}
...
bool RepositoryManager::saveRepository(const string fileName) {
    ...
    return succeed("Saved " + to_string(repoClas.getFiles().size()) + " file(s) and " +
                   to_string(repoClas.getCommits().size()) + " commit(s) to '" + fileName + "'.");
}
//DATA MANAGER CALL
...
bool RepositoryManager::diffFileAgainstCommit(const string commitID, const string filePath, string& outDiff) {
...
bool RepositoryManager::getFileContent(const string filePath, string& outContent) {
