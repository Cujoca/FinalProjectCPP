 ...
 *   - it records a human-readable explanation of what happened in lastMessage(),
 *     so main.cpp can hand that straight to ConsoleView without having to guess
 *     why a bool came back false.
 */
...
public:
    ...
    const vector<unique_ptr<Commit>>& getCommits() const { return repoClas.getCommits(); }
    ...
    const Commit* findCommit(const string commitID) const { return repoClas.findCommit(commitID); }

    // ----- analytics -----
    int    getTotalCommits()      { return commitAnalytics.computeTotalCommits(repoClas.getCommits()); }
    int    getTrackedFileCount()  { return fileAnalytics.computeTrackedFilesCount(repoClas.getFiles()); }
    string getMostModifiedFile()  { return commitAnalytics.computeMostModifiedFiles(repoClas.getCommits()); }
