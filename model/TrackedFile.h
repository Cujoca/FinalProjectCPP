...
enum class Status { Modified, Staged, Committed, Error };
...
// Just a simple redirector overload for status enum. Essentially a toString
// wrapper
...
public:
  ...
  // getters
  // NOTE: const so a TrackedFile can be read through a const reference — the
  // view and the analytics engine both inspect files without copying them.
