 ...
 * returns the appropriate error type if encountered, the trimmed repo name if
 * not
 *
...
expected<string, Error> Validator::validateRepoName(const std::string &name) {
  ...
  if (trimmed.length() < 3) {
    return unexpected(Error::TooShort);
  }
  if (trimmed.length() > 50) {
    return unexpected(Error::TooLong);
  }
  if (regex_match(trimmed, oneAlpha)) {
    return unexpected(Error::NoAlpha);
  }
...
expected<string, Error> Validator::validateRepoPath(const std::string &path) {
  ...
  if (!regex_match(trimmed, windowsPath) &&
      !regex_match(trimmed, relativePath)) {
...
expected<string, Error> Validator::validateCommitID(const std::string &id) {
  ...
  if (trimmed.length() > 40)
    return unexpected(Error::TooLong);
  if (!regex_match(trimmed, idPattern))
    return unexpected(Error::InvalidPath);
...
// std::optional<Error> Validator::validateStatusTransition(std::string name)
